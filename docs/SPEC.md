...
- **Remark place names** ("City, ST") and **time zones** come from an **offline** nearest-place dataset. Build it from GeoNames `cities500.zip` (or `cities1000.zip`) filtered to US + CA, with US state and Canadian province abbreviations, and save it gzipped at `backend/trips/data/places.csv.gz` (name, admin abbrev, lat, lon, population, IANA `tz`). `timezone_at(lat, lon)` returns the nearest place's zone ("UTC" when nothing is in range); `tzdata` is a dependency so the zones resolve on Vercel. Commit the build script to `backend/scripts/build_places.py`. Look places up with a simple lat/lon grid-bucket index. Avoid reverse-geocoding APIs for this: they are rate-limited.
...
Before any on-duty activity (drive, pickup, dropoff, fuel) when `window_start is None`, **start a duty period**. First, if a restart is needed (below), take a RESTART. Then set `window_start = t` and do PRE_TRIP if enabled. If fuel was due before the rest but there was no cycle room to take it, fuel right after the pre-trip, before driving off.
...
2. Else if `drive_in_period >= 11 h` or `t - window_start >= 14 h`: if fuel is due within FUEL_EARLY_MILES, or within FUEL_BEFORE_REST_DRIVING of the next period's driving (and fueling now adds no fuel stop), FUEL first; take POST_TRIP (if enabled), then REST (10 h, `rest_status`), or RESTART per the restart decision and search. Reset the period.
...
- `POST /api/trips/plan/`: body is `PlanRequest`, response is `PlanResponse`.
  ...
  - Name every event and stop location with the offline dataset (plus the road ref when the stop is on a highway).
  ...
  - Build text instructions from the Valhalla maneuvers (mapped to OSRM's maneuver vocabulary; a roundabout exit or a "continue" on the same road folds into the line before it), or from the OSRM steps on the fallback. Write your own English generator in `services/instructions.py` covering depart, arrive, turn, new name, continue, merge, on ramp, off ramp, fork, end of road, roundabout/rotary (with exit number), use lane, notification, and the modifiers. Collapse consecutive trivial steps.
...
- Pass the view's total time budget to the engine and services. Aim for under 8 s for a 3,000-mile trip on a warm function (Valhalla with a split leg takes about 6 s).
...
- **Remarks:** a second hour ruler under the grid, labeled "REMARKS".
  ...
  - Stagger labels to avoid overlap when remarks are close together. Remarks at the same place starting within 60 min share one label, activities joined with " / " and abbreviated ("Joplin, MO — Post-trip / 10-h rest (SB)"), each keeping its bracket. A long label wraps to "City, ST" and the activity on the next line, then shrinks (floor 6.5 px); remarks are never truncated; remark locations are city/state only. Leaders are drawn under the labels, and labels carry a paper halo.
