...
import { LoaderCircle, Maximize2, Sparkles } from 'lucide-react'
...
export function TripMap({ plan, preview, focus, selectedStopId, onSelectStop, loading, slow = false, onLoadSample }: TripMapProps) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-[#eef0f3]">
      ...
      {plan && <MapLegend ref={legendRef} plan={plan} open={legendOpen} onToggle={() => setLegendOpen((o) => !o)} />}
      ...
      {mapStyle && (
        <MapGL
          ...
          onLoad={(e) => {
            ...
            const el = e.target.getContainer()
      ...
      {!loaded && !loading && (
        <p className="pointer-events-none absolute inset-0 grid place-items-center text-xs font-medium text-ink-400">Loading map…</p>
      )}
